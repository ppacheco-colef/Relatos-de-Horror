...
# Limpiar entorno (opcional, para empezar fresco)
rm(list = ls())
...
# ============================================================================
# 1. CARGA DE ARCHIVOS TXT (ADAPTADO DE PDF A TXT)
# ============================================================================

# Configurar ruta a tus archivos TXT anónimos
ruta_txt <- "C:/Users/CEPYGICT/Documents/ProyectoTextMining/02_textos_anonimos"

# Verificar que la carpeta existe
if (!dir.exists(ruta_txt)) {
  stop("¡No encuentro la carpeta con los TXT! Verifica la ruta: ", ruta_txt)
}

# Listar archivos TXT (nombres anónimos: RdH0001.txt, RdH0002.txt, etc.)
...
# Función robusta para leer TXT con diferentes encodings
...
# Cargar todos los textos (lista con 4184 elementos)
...
limpiar_conservador <- function(texto) {
  ...
  texto_limpio <- texto %>%
    # 0. Normalizar espacios primero
    str_squish() %>%
    
    # 1. Normalizar espacios alrededor de puntos
    str_replace_all("\\s+\\.\\s+", " . ") %>%
    
    # 2. Convertir a minúsculas
    tolower() %>%
    
    # 3. Mantener puntuación básica
    str_remove_all("[¡!¿?;]") %>%
    ...
    # 5. QUITAR TODOS LOS NÚMEROS (¡SIMPLE!)
...
# B) LIMPIEZA PARA PALABRAS - TAMBIÉN SIN NÚMEROS
...
# ---- 2. CREAR frases_rdh (CON NOMBRES ANÓNIMOS TXT) ----

# MODIFICA la creación de frases_rdh:
frases_rdh <- tibble(
...
# ---- 3. CREAR palabras_rdh_base (CON COLUMNA archivo) ----

palabras_rdh_base <- tibble(
  archivo = relatos_de_horror,      # Nombres de archivos
  texto_original = rdh_txt          # Textos ORIGINALES (lista)
) %>%
  # Aplicar limpieza para palabras
  mutate(
...
# ============================================================================
# 2. ANÁLISIS DE FRECUENCIAS (Limpieza Agresiva)
# ============================================================================

# ---- 2.1. Crear Corpus para análisis de frecuencias ----
rdh_db <- VCorpus(VectorSource(rdh_txt))

# Función de limpieza AGRESIVA (para frecuencias/nubes de palabras)
...
# ---- 2.2. Crear TDM y calcular frecuencias ----
rdh_TDM <- TermDocumentMatrix(rdh_db, control = list(bounds = list(global = c(1, Inf))))

# Calcular frecuencias totales por palabra
frecuencias <- rowSums(as.matrix(rdh_TDM))

# Convertir a dataframe (OBJETO CLAVE: frecuencias_reales)
...
# Preparar datos para nube de palabras (usando frecuencias_reales)
rdh_d <- frecuencias_reales %>%
  filter(!term %in% c("ahí", "así")) %>%
  rename(word = term, freq = freq)


# Paleta de colores (rocket de viridis ajustada)
colores_rocket <- rocket(6)

# Nube de palabras (versión final)
set.seed(1234)
wordcloud(
  words = rdh_d$word,
  freq = rdh_d$freq,
...
# ============================================================================
# 3. FRECUENCIA DE TEMAS Y NÚMERO DE RELATOS (Barras dobles)
# ============================================================================

# ---- 3.1. Definir temas (como en tu código original) ----
temas_rdh <- list(
...
# ---- 3.2. Función para agrupar frecuencias ----
...
# ---- 3.3. Calcular frecuencias totales por grupo ----
frecuencias_agrupadas <- agrupar_frecuencias(temas_rdh, frecuencias_reales)

# ---- 3.4. Calcular número de archivos que contienen cada tema ----
# Usamos rdh_txt (textos crudos) para búsqueda exacta
# ---- 3.4. Calcular número de DOCUMENTOS que contienen cada tema ----
docs_por_grupo <- sapply(names(temas_rdh), function(grupo) {
  palabras <- temas_rdh[[grupo]]
  
  # Contar en cuántos documentos aparece al menos una palabra del grupo
...
# Convertir a dataframe (cambiar nombre de columna)
docs_por_grupo_df <- data.frame(
  grupo = names(temas_rdh),
  documentos = docs_por_grupo,  # ← Cambié "pdfs" por "documentos"
  stringsAsFactors = FALSE
)

# ---- 3.5. Combinar datos (ajustar nombres) ----
...
# ---- 3.6. Gráfico de barras doble (nombres corregidos) ----
...
# ============================================================================
# 4. BIGRAMAS Y REDES SEMÁNTICAS
# ============================================================================

# ---- 4.1. Extraer bigramas ----
rdh_bigramas <- rdh_db %>%
  tidy() %>%
...
# ---- 4.3. Gráfico de red (versión púrpura/verde) ----
...
#---- 5.1. RED DEL COMPONENTE MÁS CONECTADO ----

# Filtrar componente gigante
...
threshold <- 100  # Mismo threshold que usaste
...
# ---- 5.2. BIGRAMAS DE TEMAS (Red normalizada) ----
# Normalizar palabras clave a temas
...
# Filtrar bigramas (con tus umbrales específicos)
...
# Eliminar aristas de "sombras" con n <= 10 (tu ajuste manual)
...
# Gráfico de red de temas (versión final púrpura)
...
# Normalizar bigramas por sentidos
...
# Filtrar con umbrales específicos#
...
# Gráfico de sentidos (versión verde/morada)
...
# Ya tienes senticon_lexicon cargado de antes
...
analizar_por_lexico_corregido <- function(lexico) {
  ...
  if (lexico == "afinn") {
    # AFINN español tiene: word, value
  ...
  if (lexico == "bing") {
    # Bing español tiene: word, value, sentiment
    resultado <- palabras_rdh_base %>%
  ...
  if (lexico == "nrc") {
    # NRC español tiene: word, sentiment, value
    resultado <- palabras_rdh_base %>%
  ...
  if (lexico == "senticon") {
    # SENTICON tiene: palabra, value, palabra_normalizada, sentiment
...
cat("   Total de filas en comparacion_lexicos:", nrow(comparacion_lexicos), "\n")
cat("   Métodos únicos:", paste(unique(comparacion_lexicos$metodo), collapse = ", "), "\n")


# Aplicar a los 4 léxicos
...
# ---- 6.5. Resumen global comparativo ----
...
# ---- 6.3. HISTOGRAMA de distribución de sentimientos (NRC por frases) ----
sentimientos_por_frase <- get_sentiment(frases_rdh$frase, method = "nrc", language = "spanish")

# Crear dataframe con los sentimientos
frases_rdh_df <- frases_rdh %>%
  mutate(sentimiento = sentimientos_por_frase)

# Histograma de distribución de sentimientos (positivo/negativo)
...
# ---- 6.4. ANÁLISIS DE EMOCIONES DISCRETAS (8 emociones NRC) ----
# Por PALABRAS (no por frases) - Excluyendo positivo/negative
emociones_palabras <- palabras_rdh_base %>%
  inner_join(nrc_lex, by = c("palabra" = "word"))

# Filtrar SOLO las 8 emociones discretas (excluir positive/negative)
...
# ---- 7.1. FUNCIÓN PARA ASIGNAR PALABRAS ÚNICAS A EMOCIONES ----
# (De tu código original: palabras van a la emoción donde son más frecuentes)

asignar_palabras_unicas <- function(datos_treemap_base) {
  # 1. Para cada palabra, encontrar la emoción donde es más frecuente
...
if (!exists("emociones_por_palabra")) {
  emociones_por_palabra <- palabras_rdh_base %>%
    # 1. ELIMINAR cualquier columna 'emocion' o 'sentiment' que ya exista
    select(-any_of(c("emocion", "sentiment"))) %>%
    # 2. Hacer el join
    inner_join(nrc_lex, 
               by = c("palabra" = "word"),
               relationship = "many-to-many") %>%
    # 3. Filtrar primero (más eficiente)
    filter(sentiment %in% c("anger", "anticipation", "disgust", "fear", 
                            "joy", "sadness", "surprise", "trust")) %>%
    # 4. FINALMENTE renombrar (ahora no hay conflicto)
...
# ---- 7.3. TREEMAP 1: TOP 10 SIN DUPLICADOS (FRECUENCIAS TOTALES) ----

# 1. Asignar palabras únicas
palabras_unicas_t1 <- asignar_palabras_unicas(datos_treemap_base)

# 2. Lista de palabras a excluir (tus ajustes)
palabras_excluir <- c("cuenta", "ver", "miedo", "mamá", "madre", "padre", 
                      ...
                      "puesto", "tan", "si", "dijo")  # Agregué más de tu código

# 3. Aplicar ajustes manuales específicos
...
# 4. Tomar top 10 por emoción (ya sin duplicados)
...
# 6. Gráfico Treemap 1 (VIRIDIS + ROJO para ira)
...
# ---- 7.4. TREEMAP 2: FRECUENCIAS MEDIAS SIN DUPLICADOS ----
...
# ============================================================================
# 8. ANÁLISIS DE EMOCIONES POR FIGURAS FAMILIARES
# ============================================================================

# ---- 8.1. Definir categorías familiares ----
...
## ---- 7.2. Identificar frases con familiares ----
...
palabras_familiares <- frases_con_familiares %>%
  ...
  inner_join(
    nrc_lex %>% 
      filter(sentiment %in% c("anger", "anticipation", "disgust", "fear", 
                              "joy", "sadness", "surprise", "trust")) %>%
      distinct(word, sentiment),  # <- DISTINCT para evitar duplicados
    by = c("palabra" = "word")
  ) %>%
  rename(emocion = sentiment) %>%
  # AHORA contar
...
# ---- 8.4. Crear resumen_final CORRECTO ----
...
# Verificar resumen_final
cat("=== RESUMEN_FINAL CREADO ===\n")
cat("Estructura:\n")
print(str(resumen_final))
cat("\nPrimeras filas:\n")
print(head(resumen_final))

# ---- 8.5. Densidad emocional (CORREGIDO) ----
# frecuencias_totales: TOTAL de frases por categoría (ya lo tienes)
frecuencias_totales <- frases_con_familiares %>%
  group_by(categoria) %>%
  summarise(total_frases = n())

# Unir y calcular densidad
...
# ============================================================================
# 9. TABLA FINAL Y RESULTADOS
# ============================================================================

# ---- 9.1. TABLA FINAL (como tu paper) ----
# Calcular tabla de densidad (versión corregida)
...
cat("\n=== TABLA FINAL: DENSIDAD EMOCIONAL ===\n")
print(tabla_final)
...
# ---- 10.1. Gráfico de densidad emocional ----
...
# 1. Calcular frecuencias de palabras
palabras_emocionales_freq <- palabras_familiares %>%
  rename(n = n_apariciones)

# 2. Calcular percentiles por categoría
...
cat("Total de palabras únicas en top_balanceado_familia:", 
    n_distinct(top_balanceado_familia$palabra), "\n")
cat("Distribución inicial de palabras por emoción:\n")
print(top_balanceado_familia %>% count(emocion_es, sort = TRUE))


# ---- 9.4. Generar gráficos para cada categoría ----
